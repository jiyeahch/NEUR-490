...
for file_path in txt_files:
    ...
    # tier 'R'
    subset = (
        codes.loc[codes['Tier'] == 'LA', ['start_ms', 'end_ms', 'label']]
        .dropna()
        .reset_index(drop=True)
    )

    if subset.empty or subset['end_ms'].isna().all():
        print(f"Skipping {os.path.basename(file_path)} — no valid 'LA' tier or missing end_ms.")
        continue
    ...
    output_path = os.path.join(output_folder, f"{base_name}_LA_binary.csv")
